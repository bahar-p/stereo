...
proc exper1 {myargs} {
	...
	if {$IfSGBM} {
		...
		#puts "$::adcensus $LR $f $T $nu $maxd"
		if {$noc} {
			if { [catch { exec $::adcensus "${::L}${nu}" "${::R}${nu}" $maxd $noc "${::mask_noc}${nu}" $LR $f $T } msg] } {
				puts "Something seems to have gone wrong:"
				puts "Information about it: $::errorInfo and $msg"
			}
		} else {
			if { [catch { exec $::adcensus "${::L}${nu}" "${::R}${nu}" $maxd $noc "${::mask_occ}${nu}" $LR $f $T } msg] } {
				puts "Something seems to have gone wrong:"
				puts "Information about it: $::errorInfo and $msg"
			}
		}
