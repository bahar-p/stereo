...
{P}^{"}_{y} &= {P}^{'}_{y}
\end{align}
where $d(x,y)$ is the disparity function dependent on variables x and y that can be chosen based on the coordinate of the pixel from which the displacement is
calculated.
...
with a circularly symmetric, i.e. isotropic, 2D kernel function such as the Gaussian filter, which is normally used in most edge detection algorithms as a pre-processing step. 
The Gaussian kernel has the following form \cite{sze11}:
