...
Theoretically, corresponding pixels in stereo images are the ones that represent the same point in the real
...
{P}^{"}_{y} &= {P}^{'}_{y}
\end{align}
where $d(x,y)$ is the disparity function. Therefore, based on the aforementioned formulas, the depth of points in 3D space can be easily calculated after finding the corresponding pixels in multiple views and consequently their
...
This feature can particularly be found in the Gaussian filter, since it employs a 2D symmetric kernel. Because of this unique feature, 
