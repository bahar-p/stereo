...
The epipolar geometry in stereo images is defined by the intersection between the image planes and any plane having the line between two camera centres, known as {\it baseline}, 
as axis \cite{hart2000}. 
...
in stereo matching algorithms. Let us call a plane traversing all these points $S$.
Now, we can define the problem of finding the correspondence, as when we only know the position of the point $P'$ in the left image and want to find its correspondence, $P''$, 
in the right plane. 
According to the aforementioned property, $P'$,
its correspondence and their projection in 3D space are all coplanar with camera centres. This plane, $S$, is defined by the line connecting two camera centres and 
the rays passing through $P'$ and the left camera centre. Therefore, the point $P''$ would lie somewhere on the line created by the intersection of $S$ and the right image plane.
This line is in fact the back-projection in the right image plane of the ray passing through $P'$, $P$ and the left camera centre. This line is called the {\it epipolar line} 
corresponding to $P'$ \cite{hart2000}. 
Hence, in a stereo matching algorithm
we only need to search this line rather than the whole image in order to find the correspondence for point $P'$.
...
Here, we explain these concepts briefly. Figure \textbf{FIGURE NUMER} also provides an illustation of these concepts.
\newline
As described in \cite{hart2000}, the fundamental entities in epipolar geometry are as follows:
\begin{itemize}
...
\item The \textbf{epipolar plane}: Any plane containing the line between camera centres is called the epipolar plane. A collection of these planes is called an epipolar pencil. 
...
it is better to have the input image pairs warped first, i.e. {\it rectified} \cite{sze11}. The best way to accomplish this task is to
...
This relation is presented as follows \cite{sze11}:
\begin{equation}
d = f\frac{B}{Z}
\end{equation}
...
addressed, such as finding the depth values in textureless areas, depth discontinuities, and occluded regions \cite{sch02,bro03}.
...
Due to their importance in defining the boundaries of objects and also their correspondence to occlusion occurrence and depth discontinuities in 3D, we have also 
...
specific algorithms. Here, we will describe how it is possible to find the most salient edges in the scene using vision techniques and how closely it matches the human perception.
...
There are a variety of low-pass filters for image smoothing (blurring), e.g. {\it Sobel, or Gaussian filter}. These types of filters are known as {\it linear filters} \cite{sze11}. 
In linear filtering operators, for each pixel, a weighted combination of pixels in its neighborhood 
is used in order to estimate its final value \cite{sze11}. This process can be easily modelled by convolution of the input signal with a particular kernel function. 
\begin{equation}
...
g=f\bigotimes h
\end{equation}
where $f$ and $g$ are the input and output signals respectively, and $h$ is the kernel function which can be different depending on the filter. 
...
we do not want to apply a filter which is biased towards a particular direction in filtering out the noise. Instead, a 2 dimensional symmetric kernel is preferable. 
This feature is particularly found in Gaussian filter, since it employs a 2D symmetric kernel. Therefore, Gaussian filter is normally used in most edge detection algorithms.
...
A thorough description of Gaussian and other types of filters can be found in chapter 3 of \cite{sze11}.
...
the signal with a pair of convolution masks in each direction, to detect both horizontal and vertical edges. An edge extractor operator called {\it Sobel} is normally used
for this purpose \cite{sze11}. 
Sobel convolution kernels for both x and y directions are as follows \cite{sze11}:
...
The derivative and the direction of the edge element can then be found by \cite{sze11}:
\begin{align}
G=\sqrt{G_{x}^{2} + G_{y}^{2}} \\
\theta = \arctan (\frac{G_{y}}{G_{x}})
\end{align}

This process is equivalent to getting the first or second order directional derivative of the smoothed image and then look for zero crossings, i.e. where the sign of
the function changes \cite{sze11}. 
After finding the edge points, it is desirable to have them linked to form continuous contours. Since adjacent edge elements share common endpoints, this can be easily achieved by linking
...
{\it Canny} edge detector, proposed by John F. Canny in 1986 \cite{canny86}, is one of the most commonly used edge detection methods. There are also two different thresholds defined 
...
values above the lower threshold will be output \cite{canny86}. This process reduces the streaking effect to a significant amount \cite{canny86}. \newline
...
a weighted combination of its neighboring pixels . 
{\it Median filter, Bilateral filter, and anisotropic diffusion} are all known types of non-linear filters. Non-linear filters are normally used for image enhancement or processing binary
images \cite{sze11}. Binary images, as their name indicates, consist of merely two values, 0 or 1. These images are usually the outcome of filtering an image by a threshold and are
often used for {\it masking} operations \cite{sze11}. Operations on binary images that tend to manipulate and change the shape of the image are known as {\it morphological operations}.
...
{\it Dilation} and {\it Erosion} are two of the most basic morphological operations in binary image processing.
