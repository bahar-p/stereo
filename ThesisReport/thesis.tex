...
Understanding the fandumental concepts of the corresponding geometry of stereo matching helps to better understand the functionality of the methods designed to addressing this problem, 
thus comprehending how a 3D model can be constructed from stereo image pairs. Therefore, we will thoroughly describe the basic geometry of stereo matching in this section. \newline
If we consider two cameras that are looking at a particular scene from slightly different view points, a back projection of a point in the 3D space through rays passing each camera centre
would result in two distinct points on each image plane. From here on, we will call the point in space $P$ and its image on the first (left) and second (right) planes 
$P^{'}$ and ${P}^{"}$ respectively. \newline
As a result of $P$'s back projection on the image planes, an important property will form between the points and the camera centres, which is a plane, called {\it epipolar plane}, 
that traverses $P^{'}$, $P^{"}$ and the camera centres, thus intersecting each image plane. 
It should be emphasized that this property, from which the consequent properties are derived, is the building block of stereo matching algorithms. 
Let us denote the specified plane by $S$ for further reference. 
Since $S$ passes through the camera centres, it also passes through the line 
that connects two camera centres. This line, which is known as the {\it baseline}, intersects each image plane at a point called the {\it the epipole}. 
...
Now, we can define the problem of stereo correspondence as a case in which the location of $P^{'}$ in the image plane is clear, while the
corresponding point $P^{"}$ is unknown; however, based on the aforementioned properties, we know that $P^{"}$ is located somewhere on the line (epipolar line) 
created by the intersection of a plane traversing the ray passing through $P^{'}$ and the first camera centre and the {\it baseline}. This line is in fact, the projection of the ray going
...
first the corresponding epipolar line, that is the back-project of the 
ray passing through $P^{'}$ and the first camera centre, must be sought. 
The projection from a point to its corresponding epipolar line can be obtains through certain transformations in space; normally a rotation and translation, \textbf {FIGNUMER}.
These transformations can be expressed
through a matrix, for further geometrical calculations, that is merely dependant on the camera's properties, not the scene \cite{hart2000}.
However, dealing with these transformations while looking for corresponding points, can increase the complexity of stereo matching algorithms to certain levels; therefore, 
in order to avoid this issue, many approaches are proposed based on the assumption that image pairs are first warped, i.e. {\it rectified} \cite{sze11}.
...
i.e. their optical axis is perpendicular to the baseline. Furthermore, it might also be necessary to have the cameras tilted so that their {\it y} axis is perpendicular to the optical axis. 
After these two steps, horizontal scanlines are equivalent to the corresponding epipolar lines. This pre-processing step significantly constrains the process of searching 
for corresponding points and eliminates certain complications in stereo matching algorithms \cite{sze11}. \newline 
Using the rectification model and epipolar geometry described earlier illustrated in figure \textbf{FIGURENUM}, 
