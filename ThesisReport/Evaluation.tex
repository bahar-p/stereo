...
According to the results, in SGBM the number of outliers over for the masked regions are more than the outliers over the whole image. 
However, ADCensus shows the opposite behavior; that is the number of outliers over the masked regions are less than 
...
in applications, such as AR applications in outdoor environments, 
where finding more accurate disparity results near depth discontinuities, object boundaries and occluded areas
is more important than the other regions in the image. However, this metric per se
