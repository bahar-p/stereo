...
Nearly all the solutions to the problem of stereo correspondence have been dealing with the trade-off between the accuracy of the results and their running time.
...
are presented in the tables \ref{tab:outlmsk} and \ref{tab:outlfull} for each age group. For simplicity, we have labelled the valid pixels
in the ground truth and the disparity results with \textbf{vG} and \textbf{vD}, respectively. \newline
...
\hline
\multicolumn{4}{ |c| }{Average outliers} \\
\hline
...
\begin{minipage}{0.8\linewidth}
\begin{center}
\captionof{table}{The average outliers for the whole image}
\label{tab:outlfull}
\begin{tabular}{ |c|c|c|c| }
\hline
\multicolumn{4}{ |c| }{Average outliers} \\
...
is more important than the other regions in the image, such as image compositing for layering visual elements, such as signage
...
in the ground truth is that in our implemention, the total number of pixels for the ground truth image 
...
however, the total number of pixels in the generated disparity
is only incremented whenever the disparity value is valid and the amount of disparity error is less than the specified threshold in the evaluation process, thus resulting
...
\begin{minipage}{0.8\linewidth}
\begin{center}
\captionof{table}{The average disparity error}
\label{tab:avgerr}
\begin{tabular}{ |c|c|c| }
\hline
