...
\textbf{ADE Estimation}; START
   ...
   \(error\sb{avg} = (error\sb{total})/(pix\_count)\);
\textbf{ADE Estimation}; END
\end{alltt}

\subsection{The Average Outliers}
...
\textbf{AO Estimation}; START
   ...
   if (masked)
      for all pixels p in the masked image; do
         ...
         \(stAct\sb{err} = \frac{pupil\_distance*depth\sb{err}}{depth\sb{gt}\sp{2}}\);
	 if (\(stAct\sb{err} \geq stAct\sb{threshold}\))
            \(outlier\sb{total} += 1\);
	 fi
      end for
   else 
   ...
   \(outlier\sb{avg} = (outlier\sb{total})/(pix\_count)\);
...
\textbf{ASA Estimation}; START
   ...
   for each stActFile; do
      histImg = histogram.build(bins,width);
      Write(histImg, img\_histF);
   end for
   //Histogram for final stereoacuity over all the images
   histFinal = histogram.build(bins,width);
   Write(histFinal,final\_histF);
   plot(final\_histF);
