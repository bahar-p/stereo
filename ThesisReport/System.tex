...
metrics are measured based on the relevant factors described earlier in chapters \ref{chap:Introduction} and \ref{chap:BinocularVision} as perceived
...
This in turn permits a higher quality combination of the depth map of the real world with the virtual depth of the synthetic objects that are part of the AR scene.
...
As can be seen in the figure \ref{fig:architecture}, first the input data consisting of the stereo images, the ground truth disparity and 
the calibration data is passed in to the system.
...
for the ground truth disparity and the disparity generated by the algorithm in equation \ref{eq:dispeq}, and then compared to the relative 
...
four different values are eventually estimated. This process is repeated for all the pixels in the image or 
...
the corresponding stereoacuities. This process is done for all the pixels in the image; or if a mask has been provided, 
...
Moreover, the number of bins along with their corresponding width define the total distance over which the results
