...
int image::costAD_caller(const cv::gpu::PtrStepsz<uchar3>& srcL, const cv::gpu::PtrStepsz<uchar3>& srcR, const cv::gpu::PtrStepSz<double>& DSI,int d_idx, bool r_disp) {
	unsigned int block_dimX = (unsigned int)std::ceil(srcL.cols/(float)8);
	unsigned int block_dimY = (unsigned int)std::ceil(srcL.rows/(float)8);
...
static __global__ void costAD(cv::gpu::PtrStepsz<uchar3>srcL, cv::gpu::PtrStepsz<uchar3>srcR, cv::gpu::PtrStepsz<double> DSI,int RW, int RH,int min_d,int max_d,int d_idx, int channels, bool r_disp){

	int q = threadIdx.x + blockIdx.x * blockDim.x;
	int p = threadIdx.y + blockIdx.y * blockDim.y;
	if( q >= src.cols || p >= src.rows){
		return;
	}
	double val =0;
	int drange = max_d-min_d+1;
...
/*	for(int d=0; d< drange; d++){
		DSI[d] = cv::Mat(s, mytype,cv::Scalar::all(0));
				...
				val = 0;
				if(!(r_disp)){
					if(q-d_idx-min_d>RW-1){
						if(channels==1){
							DSI(p,q) = (double) (abs(srcL(p,q) - srcR(p,q-d_idx-min_d)))/(double)channels;
						}
						else if (channels == 3 || channels == 4) {
							DSI(p,q)= (double)((abs(srcL(p,q).x - srcR(p,q-d_idx-min_d).x)) + 
							(abs(srcL(p,q).y - srcR(p,q-d_idx-min_d).y)) +
							(abs(srcL(p,q).z - srcR(p,q-d_idx-min_d).z)))/3;
						} 
						/*else {
							std::cerr << "Unhandled number of channels " << endl;
					...
					//Right disparity
					if(q+d_idx+min_d<srcL.cols-RW){
						
						if(channels==1){
							DSI(p,q) = (double) (abs(srcL(p,q+d_idx+min_d)- srcR(p,q)))/(double)channels;
						}
						else if (channels == 3 || channels == 4){
							DSI(p,q)=(double)((abs(srcL(p,q+d_idx+min_d).x - srcR(p,q).x)) + 
							(abs(srcL(p,q+d_idx+min_d).y - srcR(p,q).y)) +
							(abs(srcL(p,q+d_idx+min_d).z - srcR(p,q).z)))/3;
						}
					}
				}
