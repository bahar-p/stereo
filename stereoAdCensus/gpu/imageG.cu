...
static __global__ void costAD(cv::gpu::PtrStepSz<uchar3>srcL, cv::gpu::PtrStepSz<uchar3>srcR, cv::gpu::PtrStepSz<double> DSI,int RW, int RH,int min_d,int max_d,int d_idx, int chn, bool r_disp){

	int q = threadIdx.x + blockIdx.x * blockDim.x;
	int p = threadIdx.y + blockIdx.y * blockDim.y;
	if( q >= srcL.cols || p >= srcL.rows){
		return;
	}
...
//	cv::Mat* DSI = new cv::Mat[drange];
	...
	if(!(r_disp)){
		if(q-d_idx-min_d>RW-1){
			//if(chn==1){
				//DSI(p,q) = (float)(abs(srcL(p,q) - srcR(p,q-d_idx-min_d)))/(float)chn;
			//}
		//	else if (chn == 3 || chn == 4) {
	...
	//Right disparity
		if(q+d_idx+min_d<srcL.cols-RW){
			//if(chn==1){
			//	DSI(p,q) = (float) (abs(srcL(p,q+d_idx+min_d)- srcR(p,q)))/(float)chn;
			//}
			//else if (chn == 3 || chn == 4){
...
void image::findOutliers(cv::Mat dispL, cv::Mat dispR,cv::Mat& pixflag, float f, float B){
