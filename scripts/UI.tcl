...
grid .f1.l1 -column 0 -row $r
grid .f1.cb1 -column 1 -row $r
grid .f1.cb2 -column 2 -row $r
grid .f1.cb3 -column 3 -row $r

#increment row
incr r

label .f1.l5 -text "Call Evaluation:	"
...
grid .f1.l5 -column 0 -row $r
grid .f1.sp1 -column 1 -row $r
grid .f1.b1 -column 2 -row $r

#increment row
incr r
label .f1.l2 -text "Avg StAcuity/img:	"
...
button .f1.b2 -text "Call hist" -command {hist1 $noc $bins $width $adc}
grid .f1.l2 -column 0 -row $r
grid .f1.sp2 -column 1 -row $r
grid .f1.sp3 -column 2 -row $r
grid .f1.b2 -column 3 -row $r
...
button .f1.b3 -text "Call histfinal" -command {hist2 $bins $width $noc $imgcnt}

grid .f1.l3 -column 0 -row $r
grid .f1.sp4 -column 1 -row $r
grid .f1.b3 -column 2 -row $r

#increment row
incr r

#label .f1.l4 -text "Avg StaAcuity plot"
button .f1.b4 -width 10 -text "Plot" -command {plot $noc}

incr r
puts "r: $r"
#grid .f1.l4 -column 0 -row $r
grid .f1.b4 -column 2 -row $r
...
proc hist2 {bins width noc imgcnt} {
	if {$noc} {
		fhist $bins $width  "../eval/results/noc/histimgs.txt" $imgcnt;
	} else {
		fhist $bins $width  "../eval/results/occ/histimgs.txt" $imgcnt;
	}
}
proc plot {noc} {
	if {$noc} {
		finalplot "../eval/results/noc/histfinal.txt"
	} else {
		finalplot "../eval/results/occ/histfinal.txt"
	}
}
